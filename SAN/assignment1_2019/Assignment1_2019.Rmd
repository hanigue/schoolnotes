...
ggplot(Boston, aes(lstat, medv)) + geom_point() + stat_smooth(method = lm, formula = y ~ x) # visualizace medv a lstat 
...
summary(poly) # pokus o precteni summary z polynomialniho modelu

ggplot(Boston, aes(rm, medv) ) + geom_point() + stat_smooth(method = lm, formula = y ~ poly(x, 5, raw = TRUE)) # vizualizace polynomialniho modelu rm a medv
ggplot(Boston, aes(lstat, medv) ) + geom_point() + stat_smooth(method = lm, formula = y ~ poly(x, 5, raw = TRUE)) # vizualizace lstat a medv
...
summary(poly2) # Pokus o summary 2 - polynom stupne 9 ma sice nejmensi MSE, ale podle summary ma hodne "nevyznamnych" clenu
# taky nevime, jestli overfituje, protoze stupen 10 uz mi to nespocita
...
# U zbytku se MSE zlepsuje, kdyz zvysuju stupne
# Dalsi moznost - polynom 2 MSE = 21.3 a polynom 3 = 21.6, ale rozdil byl mnohem mensi nez u 6 a 7
# Je zajimave, ze kdyz pokracuju 8 a 9, tak se to zase zlepsuje
```
...
# Pokus o snížení MSE, když budu nějakým způsobem trochu následovat vizualizaci dat. Parametry lstat a medv vůči sobě vypadaly, že by to mohla být logaritmická funkce, rm a medv už byly spíše polynom (log v obou parametrech dost zhoršil).
# Finální MSE: 19.47
...
Obecně na tom GAM modely byly trochu hůř, ale ne o moc (kromě gam, kde byl jeden člen lineární, který byl výrazně horší). Nelze se ovšem orientovat pouze podle MSE, jelikož máme velmi malé množství dat, které se ještě k tomu rozdělilo na trénovací a testovací. Je to zřejmé hlavně u polynomů, při kterých se MSE měnilo bizardním způsobem při změně seedu. Polynomy jsou jinak obecně méně stabilní a hlavně by se neměl používat tak vysoký řad. 
...
Největší je rozdíl má "nejlepší" model s lineárním modelem, pak také s modelem gam, který má lineární složku. Se zbytkem rozdíl není příliš velký. Jak již bylo zmíněno, pouze na MSE se nelze spolehnout, MSE se liší často o málo a výsledek se velmi odlišuje pokaždé, když je vygenerován nový seed. Jinak bych statistickou významnost rozdílů v MSE zjistila pomocí t-testu. 
