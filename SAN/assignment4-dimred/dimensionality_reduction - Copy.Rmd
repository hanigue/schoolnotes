...
```{r}
FindBestKForISOMAP <-function(data, neighbors, colors)
{
  res <- array()
  colors <- rep("black", 20)
  for (k in 2:10) {
    g <- ConstructGraph(data, neighbors, k)
    paths <- floyd(g)
    if (is.finite(sum(paths))){
      fit <- cmdscale(paths, eig=TRUE, k=2)
      accuracy <- mean(Classify(as.data.frame(fit$points), mycolors$V1, 50))
      res[k] <- mean(accuracy)
    } else {
      res[k] <- 0
      colors[k] <- "grey"
  ...
  colors[which.max(res[3:20])+2] <- "green"
  colors[which.min(res[3:20])+2] <- "red"
  plot(1:length(res), res, col = colors, xlab="k", ylab="Classification accuracy", type = "b", pch = 16, main = "Classification accuracy plot")
  return(res)
}
result <- FindBestKForISOMAP(mydata, myneighbors, mycolors)
